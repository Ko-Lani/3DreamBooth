...
class MMDoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        #     cond_img_modulated = modulate(cond_img_modulated, shift=img_mod1_shift, scale=img_mod1_scale)
        ...
        with specify_lora([self.img_attn_q, self.img_attn_k, self.img_attn_v], None):
        ...
        if use_cond:
            with specify_lora([self.img_attn_q, self.img_attn_k, self.img_attn_v], "tdapter"):
                cond_img_q = self.img_attn_q(cond_img_modulated)
                cond_img_k = self.img_attn_k(cond_img_modulated)
                cond_img_v = self.img_attn_v(cond_img_modulated)
                cond_img_q = rearrange(cond_img_q, "B L (H D) -> B L H D", H=self.heads_num)
                cond_img_k = rearrange(cond_img_k, "B L (H D) -> B L H D", H=self.heads_num)
                cond_img_v = rearrange(cond_img_v, "B L (H D) -> B L H D", H=self.heads_num)
        ...
        if freqs_cis is not None:
            ...
            if use_cond:
                cond_img_qq, cond_img_kk = apply_rotary_emb(cond_img_q, cond_img_k, cond_freqs_cis, head_first=False)
                assert (
                ...
                cond_img_q, cond_img_k = cond_img_qq, cond_img_kk


                img_q = torch.cat([img_q, cond_img_q], dim=1)
                img_k = torch.cat([img_k, cond_img_k], dim=1)

                img_v = torch.cat([img_v, cond_img_v], dim=1)
        ...
        with specify_lora([self.img_mlp.fc1, self.img_mlp.fc2, self.img_attn_proj], None):
        ...
        if use_cond:
            with specify_lora([self.img_mlp.fc1, self.img_mlp.fc2, self.img_attn_proj], "tdapter"):
                # cond_img = cond_img + apply_gate(self.img_attn_proj(img_cond_attn), gate=img_mod1_gate)
                # cond_img = cond_img + apply_gate(
                #     self.img_mlp(
                #         modulate(self.img_norm2(cond_img), shift=img_mod2_shift, scale=img_mod2_scale)
                #     ),
                #     gate=img_mod2_gate,
                # )

                cond_img = cond_img + apply_gate(self.img_attn_proj(img_cond_attn), gate=cond_mod1_gate)
                cond_img = cond_img + apply_gate(
                    self.img_mlp(
                        modulate(self.img_norm2(cond_img), shift=cond_mod2_shift, scale=cond_mod2_scale)
                    ),
                    gate=cond_mod2_gate,
                )
...
class TdapterTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def forward(
        ...
        # full_freqs_cos, full_freqs_sin = self.get_rotary_pos_embed((1, th, tw))


        f_cos, f_sin = self.get_rotary_pos_embed((1, th, tw))
        ...
        cond_freqs_cos = f_cos_chunk
        cond_freqs_sin = f_sin_chunk



        with specify_lora([self.img_in.proj], None):
            img = self.img_in(img)


        if use_condition:
            with specify_lora([self.img_in.proj], "tdapter"):
                cond_img = self.img_in(cond_img)
        ...
        if sp_enabled:
            ...
            if use_condition:
                cond_img = torch.chunk(cond_img, sp_size, dim=1)[sp_rank]


            # cond_freqs_cos is already chunked above (lines ~883-884); do not re-chunk here
            # cond_freqs_cos = torch.chunk(cond_freqs_cos, sp_size, dim=0)[sp_rank]
            # cond_freqs_sin = torch.chunk(cond_freqs_sin, sp_size, dim=0)[sp_rank]
        ...
        cond_freqs_cis = (cond_freqs_cos, cond_freqs_sin) if cond_freqs_cos is not None else None
        ...
        for index, block in enumerate(self.double_blocks):
            ...
            img, txt, cond_img = block(
                img=img,
                txt=txt,
                cond_img=cond_img if use_condition else None,
                vec=vec,
                vec_cond=vec_cond,
                freqs_cis=freqs_cis,
