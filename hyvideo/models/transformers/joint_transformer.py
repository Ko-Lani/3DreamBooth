...
from .modules.tdapter_attention import parallel_attention
...
@contextmanager
def specify_lora(lora_modules: List[BaseTunerLayer], specified_loras):
    """Temporarily enable only the requested active LoRA adapters."""
    if isinstance(specified_loras, str):
        specified_loras = [specified_loras]

    valid_lora_modules = [m for m in lora_modules if isinstance(m, BaseTunerLayer)]

    original_scales = [
        {adapter: module.scaling[adapter] for adapter in module.active_adapters if adapter in module.scaling}
    ...
    for module in valid_lora_modules:
        for adapter in module.active_adapters:
            if adapter in module.scaling:
                module.scaling[adapter] = 1 if adapter in specified_loras else 0
    try:
        yield
    finally:
        # Restore the original adapter scaling after the scoped override.
...
def _lora_forward_masked(modules, adapter, x, span_mask=None):
    """Apply LoRA projection(s) to x, optionally masking the delta to span positions.

    When span_mask is None, behaves identically to calling each module inside
    ``specify_lora(modules, adapter)``.  When span_mask is provided (float
    tensor of shape [B, L] or [B, L, 1], values 0/1), the LoRA delta is only
    added at positions where span_mask == 1; elsewhere the base (no-LoRA)
    output is used.

    Returns a list of outputs, one per module in *modules*.
    """
    with specify_lora(modules, adapter):
        lora_outs = [m(x) for m in modules]

    if span_mask is None:
        return lora_outs

    with specify_lora(modules, []):
        base_outs = [m(x) for m in modules]

    mask = span_mask.to(x.dtype)
    if mask.dim() == 2:
        mask = mask.unsqueeze(-1)  # [B, L, 1]

    return [b + (l - b) * mask for b, l in zip(base_outs, lora_outs)]
...
class MMDoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        img: torch.Tensor,
        txt: torch.Tensor,
        cond_imgs: Optional[List[torch.Tensor]],
        vec: torch.Tensor,
        vec_cond: torch.Tensor = None,
        freqs_cis: tuple = None,
        cond_freqs_cis: tuple = None,
        text_mask=None,
        attn_param=None,
        is_flash=False,
        block_idx=None,
        ...
        cond_imgs = cond_imgs or []
        num_cond = len(cond_imgs)
        use_cond = num_cond > 0
        ...
        # Condition branch: use vec_cond (timestep=0) for modulation
        if vec_cond is not None:
            (
                ...
                cond_mod2_scale,
                cond_mod2_gate,
            ) = self.img_mod(vec_cond).chunk(6, dim=-1)
        else:
            cond_mod1_shift, cond_mod1_scale, cond_mod1_gate = img_mod1_shift, img_mod1_scale, img_mod1_gate
            cond_mod2_shift, cond_mod2_scale, cond_mod2_gate = img_mod2_shift, img_mod2_scale, img_mod2_gate

        img_modulated = self.img_norm1(img)
        img_modulated = modulate(img_modulated, shift=img_mod1_shift, scale=img_mod1_scale)

        if use_cond:
            cond_img_modulated_list = []
            for cond_img in cond_imgs:
                cond_img_modulated = self.img_norm1(cond_img)
                cond_img_modulated = modulate(cond_img_modulated, shift=cond_mod1_shift, scale=cond_mod1_scale)
                cond_img_modulated_list.append(cond_img_modulated)

        with specify_lora([self.img_attn_q, self.img_attn_k, self.img_attn_v], "tdb"):
        ...
        if use_cond:
            with specify_lora([self.img_attn_q, self.img_attn_k, self.img_attn_v], "tdapter"):
                all_conds = torch.stack(cond_img_modulated_list, dim=0)

                cond_img_q = self.img_attn_q(all_conds)
                cond_img_k = self.img_attn_k(all_conds)
                cond_img_v = self.img_attn_v(all_conds)

                cond_img_q = rearrange(cond_img_q, "N B L (H D) -> N B L H D", H=self.heads_num)
                cond_img_k = rearrange(cond_img_k, "N B L (H D) -> N B L H D", H=self.heads_num)
                cond_img_v = rearrange(cond_img_v, "N B L (H D) -> N B L H D", H=self.heads_num)

                cond_img_q = self.img_attn_q_norm(cond_img_q).to(cond_img_v)
                cond_img_k = self.img_attn_k_norm(cond_img_k).to(cond_img_v)

                cond_img_q_list = list(torch.unbind(cond_img_q, dim=0))
                cond_img_k_list = list(torch.unbind(cond_img_k, dim=0))
                cond_img_v_list = list(torch.unbind(cond_img_v, dim=0))
        ...
        if freqs_cis is not None:
            ...
            if use_cond:
                ...
                for cond_img_q, cond_img_k, current_cis in zip(cond_img_q_list, cond_img_k_list, cond_freqs_cis):
                    cond_img_qq, cond_img_kk = apply_rotary_emb(cond_img_q, cond_img_k, current_cis, head_first=False)

                    assert (
                        cond_img_qq.shape == cond_img_q.shape and cond_img_kk.shape == cond_img_k.shape
                    ), f"cond_img_kk: {cond_img_qq.shape}, cond_img_q: {cond_img_q.shape}, cond_img_kk: {cond_img_kk.shape}, cond_img_k: {cond_img_k.shape}"
                    cond_img_qq_list.append(cond_img_qq)
                    cond_img_kk_list.append(cond_img_kk)

                img_q = torch.cat([img_q] + cond_img_qq_list, dim=1)
                img_k = torch.cat([img_k] + cond_img_kk_list, dim=1)
                img_v = torch.cat([img_v] + cond_img_v_list, dim=1)

        txt_modulated = self.txt_norm1(txt)
        txt_modulated = modulate(txt_modulated, shift=txt_mod1_shift, scale=txt_mod1_scale)

        txt_q, txt_k, txt_v = _lora_forward_masked(
            [self.txt_attn_q, self.txt_attn_k, self.txt_attn_v],
            "tdb", txt_modulated, txt_span_mask,
        )
        ...
        v_len = img.shape[1]
        c_len = cond_imgs[0].shape[1] * num_cond if use_cond else 0

        full_img_attn = attn[:, :img_q.shape[1]]
        txt_attn = attn[:, img_q.shape[1]:]

        if use_cond:
            img_attn = full_img_attn[:, :v_len]
            img_cond_attn = full_img_attn[:, v_len : v_len + c_len]
        else:
            img_attn = full_img_attn
            img_cond_attn = None



        with specify_lora([self.img_mlp.fc1, self.img_mlp.fc2, self.img_attn_proj], "tdb"):
        ...
        if use_cond:
            with specify_lora([self.img_mlp.fc1, self.img_mlp.fc2, self.img_attn_proj], "tdapter"):
                split_size = img_cond_attn.shape[1] // num_cond
                cond_list = list(img_cond_attn.split(split_size, dim=1))

                updated_cond_imgs = []

                for original_cond, attn_res in zip(cond_imgs, cond_list):

                    res = original_cond + apply_gate(self.img_attn_proj(attn_res), gate=cond_mod1_gate)

                    res = res + apply_gate(
                        self.img_mlp(
                            modulate(self.img_norm2(res), shift=cond_mod2_shift, scale=cond_mod2_scale)
                        ),
                        gate=cond_mod2_gate,
                    )
                    updated_cond_imgs.append(res)

                cond_imgs = updated_cond_imgs


        if txt_span_mask is None:
            with specify_lora([self.txt_attn_proj, self.txt_mlp.fc1, self.txt_mlp.fc2], "tdb"):
            ...
            span_mask = txt_span_mask.unsqueeze(-1).to(txt.dtype)  # [B, L, 1]

            # Proj: masked LoRA delta
            (proj_out,) = _lora_forward_masked(
                [self.txt_attn_proj], "tdb", txt_attn, txt_span_mask
            )
            txt = txt + apply_gate(proj_out, gate=txt_mod1_gate)

            # MLP: masked LoRA delta (txt_normed computed once, mlp run twice)
            txt_normed = modulate(self.txt_norm2(txt), shift=txt_mod2_shift, scale=txt_mod2_scale)
            with specify_lora([self.txt_mlp.fc1, self.txt_mlp.fc2], []):
                mlp_base = self.txt_mlp(txt_normed)
            with specify_lora([self.txt_mlp.fc1, self.txt_mlp.fc2], "tdb"):
                mlp_lora = self.txt_mlp(txt_normed)
            txt = txt + apply_gate(mlp_base + (mlp_lora - mlp_base) * span_mask, gate=txt_mod2_gate)

        return img, txt, cond_imgs
...
class JointTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def forward(
        ...
        cond_hidden_states: List[torch.Tensor],
        ...
        cond_hidden_states = cond_hidden_states or []
        num_cond = len(cond_hidden_states)
        use_condition = num_cond > 0
        ...
        for i in range(num_cond):
            ...
            f_cos, f_sin = self.get_rotary_pos_embed((target_t, th, tw))
            f_cos_chunk = f_cos[-th*tw:]
            f_sin_chunk = f_sin[-th*tw:]
            ...
            if sp_enabled:
                ...
                f_cos_chunk = torch.chunk(f_cos_chunk, sp_size, dim=0)[sp_rank]
                f_sin_chunk = torch.chunk(f_sin_chunk, sp_size, dim=0)[sp_rank]

            cond_freqs_cos_list.append(f_cos_chunk)
            cond_freqs_sin_list.append(f_sin_chunk)


        cond_freqs_cis = list(zip(cond_freqs_cos_list, cond_freqs_sin_list))

        with specify_lora([self.img_in.proj], "tdb"):
            img = self.img_in(img)

        if use_condition:
            with specify_lora([self.img_in.proj], "tdapter"):
                N = len(cond_imgs)
                all_conds = torch.stack(cond_imgs, dim=0)
                flat_conds = rearrange(all_conds, "n b c t h w -> (n b) c t h w")
                flat_processed = self.img_in(flat_conds)
                stacked_processed = rearrange(flat_processed, "(n b) l d -> n b l d", n=N)
                cond_imgs_processed = list(torch.unbind(stacked_processed, dim=0))
        ...
        if sp_enabled:
            ...
            if use_condition:
                cond_imgs_processed = [
                    torch.chunk(c, sp_size, dim=1)[sp_rank]
                    for c in cond_imgs_processed
                ]
        ...
        # Condition modulation: timestep=0 (clean reference images)
        vec_cond = self.time_in(torch.zeros_like(t))
        if text_states_2 is not None:
            vec_cond = vec_cond + vec_2
        if self.guidance_embed and guidance is not None:
            vec_cond = vec_cond + self.guidance_in(guidance)
        if timestep_r is not None:
            vec_cond = vec_cond + self.time_r_in(torch.zeros_like(timestep_r))
        ...
        if self.glyph_byT5_v2:
            ...
            # byt5 tokens are prepended; span mask must be padded with zeros at the front
            if txt_span_mask is not None:
                byt5_zeros = torch.zeros(
                    txt_span_mask.shape[0], byt5_txt.shape[1],
                    dtype=txt_span_mask.dtype, device=txt_span_mask.device,
                )
                txt_span_mask = torch.cat([byt5_zeros, txt_span_mask], dim=1)
        ...
        if self.vision_in is not None and vision_states is not None:
            ...
            # vision tokens are prepended; span mask must be padded with zeros at the front
            if txt_span_mask is not None:
                vision_zeros = torch.zeros(
                    txt_span_mask.shape[0], extra_encoder_hidden_states.shape[1],
                    dtype=txt_span_mask.dtype, device=txt_span_mask.device,
                )
                txt_span_mask = torch.cat([vision_zeros, txt_span_mask], dim=1)
        ...
        for index, block in enumerate(self.double_blocks):
            ...
            img, txt, cond_imgs_processed = block(
                img=img,
                txt=txt,
                cond_imgs=cond_imgs_processed if use_condition else None,
                vec=vec,
                vec_cond=vec_cond,
                freqs_cis=freqs_cis,
                cond_freqs_cis=cond_freqs_cis,
                text_mask=text_mask,
