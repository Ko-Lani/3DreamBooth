...
class ImageToVideoMaskDataset(Dataset):
    ...
    def __getitem__(self, index):
        pixel_values = self.instance_videos[index]  # [3, 1, H, W], range [-1, 1]
        loss_mask = self.instance_masks[index]       # [1, 1, H, W], range [0, 1]

        # Random background augmentation to prevent background overfitting
        binary_mask = (loss_mask > 0.5).float()

        _, _, h, w = pixel_values.shape
        bg_type = random.random()
        if bg_type < 0.4:
            # Random solid color (studio backdrop-like)
            random_bg = torch.rand(3, 1, 1, 1) * 2 - 1
            random_bg = random_bg.expand_as(pixel_values)
        elif bg_type < 0.7:
            # Smooth gradient (natural lighting/wall-like)
            c1 = torch.rand(3, 1, 1, 1) * 2 - 1
            c2 = torch.rand(3, 1, 1, 1) * 2 - 1
            if random.random() < 0.5:
                # Vertical gradient
                t = torch.linspace(0, 1, h).view(1, 1, h, 1).expand_as(pixel_values)
            else:
                # Horizontal gradient
                t = torch.linspace(0, 1, w).view(1, 1, 1, w).expand_as(pixel_values)
            random_bg = c1 * (1 - t) + c2 * t
        elif bg_type < 0.9:
            # Blurred soft texture (out-of-focus background feel)
            noise = torch.rand(3, 1, h // 8, w // 8) * 2 - 1
            random_bg = F.interpolate(noise, size=(h, w), mode='bilinear', align_corners=False)
        else:
            # Random uniform gray
            gray = random.random() * 2 - 1
            random_bg = torch.full_like(pixel_values, gray)

        pixel_values = pixel_values * binary_mask + random_bg * (1 - binary_mask)
