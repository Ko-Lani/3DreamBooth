...
from hyvideo.models.transformers.joint_transformer import (
    JointTransformer,
)
...
class JointPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        cond_cond_latents = self._prepare_cond_latents(task_type, image_cond, ref_latents[0], multitask_mask)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar, auto_offload_model(self.transformer, self.execution_device, enabled=self.enable_offloading):
            for i, t in enumerate(timesteps):
                ...
                #     ref_list.append(torch.cat([ref, cond_cond_latents], dim=1))

                # latents_condition = torch.cat(ref_list, dim=2)
                # condition_latent = torch.cat([latents_condition] * 2) if self.do_classifier_free_guidance else latents_condition

                # condition_latent_model_input = list(condition_latent.split(1, dim=2))

                combined_ref = [torch.cat([ref, cond_cond_latents], dim=1) for ref in ref_latents]

                # 2. 만약 CFG(Classifier Free Guidance)를 쓴다면 각 원소를 2배로 늘리기
                if self.do_classifier_free_guidance:
                    condition_latent_model_input = [torch.cat([c] * 2) for c in combined_ref]
                else:
                    condition_latent_model_input = combined_ref


                with torch.autocast(device_type="cuda", dtype=self.target_dtype, enabled=self.autocast_enabled):
                    output = self.transformer(
                        hidden_states=latent_model_input,
                        cond_hidden_states=condition_latent_model_input,
                        timestep=t_expand,
    ...
    def load_sr_transformer_upsampler(cls, cached_folder, sr_version, transformer_dtype=torch.bfloat16, device=None):
        transformer = _3dbTransformer.from_pretrained(
    ...
    def create_pipeline(cls, pretrained_model_name_or_path, transformer_version, create_sr_pipeline=False, transformer_dtype=torch.bfloat16, device=None, transformer_init_device=None, **kwargs):
        ...
        transformer = JointTransformer.from_pretrained(
            **from_pretrain_kwargs,
            torch_dtype=transformer_dtype,
            low_cpu_mem_usage=True,
