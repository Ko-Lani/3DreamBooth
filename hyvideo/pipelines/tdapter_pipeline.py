...
# )
from hyvideo.models.transformers.tdapter_transformer import (
    TdapterTransformer,
)
...
class TdapterPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        # transformer: _3dbTransformer,
        # transformer: HunyuanVideo_1_5_DiffusionTransformer,
        transformer: TdapterTransformer,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        cond_cond_latents = self._prepare_cond_latents(task_type, image_cond, ref_latents, multitask_mask)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar, auto_offload_model(self.transformer, self.execution_device, enabled=self.enable_offloading):
            for i, t in enumerate(timesteps):
                ...
                latents_condition = torch.cat([ref_latents, cond_cond_latents], dim=1)
                condition_latent_model_input = torch.cat([latents_condition] * 2) if self.do_classifier_free_guidance else latents_condition
    ...
    def create_pipeline(cls, pretrained_model_name_or_path, transformer_version, create_sr_pipeline=False, transformer_dtype=torch.bfloat16, device=None, transformer_init_device=None, **kwargs):
        ...
        transformer = TdapterTransformer.from_pretrained(
            **from_pretrain_kwargs,
            torch_dtype=transformer_dtype,
            low_cpu_mem_usage=True,
        ).to(transformer_init_device)
