...
class TrainingConfig:
    ...
    text_lora_spans: Optional[List[str]] = None  # Substrings of the prompt to restrict text LoRA to during training. None = all tokens.
...
class HunyuanVideoTrainer:
    ...
    def save_checkpoint(self, step: int):
        ...
        if self.config.use_lora and hasattr(self.transformer, "save_lora_adapter"):
            ...
            if self.is_main_process:
                logger.info(f"LoRA checkpoint saved at step {step} to {checkpoint_dir}")
            return

        # Save full model state dict (non-LoRA mode only)
    ...
    def train(self, dataloader):
        ...
        # Validate at the final step if it aligns with the validation interval
        # (the in-loop check fires before global_step is incremented, so the very
        # last step — equal to max_steps — is never reached inside the loop)
...
def create_dummy_dataloader(config: TrainingConfig):
    ...
    dataset = ImageToVideoMaskDataset(
        ...
        # width=config.width,
        # video_reshape_mode=config.video_reshape_mode,
        # fps=config.fps,
        # max_num_frames=config.max_num_frames,
        # skip_frames_start=config.skip_frames_start,
        # skip_frames_end=config.skip_frames_end,
    )
...
def main():
    ...
    parser.add_argument("--text_lora_spans", type=str, nargs="+", default=None,
                        help="Restrict text LoRA to specific token spans during training. "
                             "Provide substrings of the train prompt, e.g. --text_lora_spans 'rhs figure'. "
    ...
    parser.add_argument("--num_train_timesteps", type=int, default=1000,
                        help="Number of training timesteps for the noise scheduler (default: 1000)")
